...
namespace ViewROI
{
    ...
    public delegate void Callback_Point(double row, double col, object oUser);
    ...
    public interface ICallbackRoiPoint
    {
        void Point_Move(double row, double col, object oUser);
        void Point_Decide(double row, double col, object oUser);
        void Point_Cancel(object oUser);
    }
    ...
    public class ROIControllerCallback
    {
        ...
        private Callback_Point point_move = null;
        private Callback_Point point_decide = null;
        private Callback_Cancel point_cancel = null;
            ...
            if (val == ROIController.EVENT_MOVING_ROI)
            {
                ...
                switch (RoiMode)
                {
                    ...
                    case ROIMode.Line:
                        ...
                        break;
                    case ROIMode.Point:
                        point_move(data[0].D, data[1].D, oUserData);
                        break;
                }
            }
        }
        ...
        public bool StartRoi_Point(double row, double col, ICallbackRoiPoint rc, object user)
        {
            if (RoiMode != ROIMode.None)
                return false;
            RoiMode = ROIMode.Point;
            oUserData = user;
            point_decide = rc.Point_Decide;
            point_move = rc.Point_Move;
            point_cancel = rc.Point_Cancel;
            mRc.setROIShape(new ROIPoint(row, col));
            mRc.setActiveROIIdx(0);
            return true;
        }

        public bool UpdateRoi_Point(double row, double col, bool callback = true)
        {
            if (RoiMode != ROIMode.Point)
                return false;
            ...
            if (al.Count == 0)
                return false;
            ROI roi = (ROI)al[0];
            if (roi.RoiType != ROI.ROI_TYPE_POINT)
                return false;
            roi.setModelData(new HTuple(new double[] { row, col }));

            if (callback)
            {
                HTuple newdata = roi.getModelData();
                point_move(newdata[0].D, newdata[1].D, oUserData);
            }
        ...
        public bool DecideRoi_Point()
        {
            ...
            mRc.reset();
            mRc.viewController.repaint();
            point_decide(data[0].D, data[1].D, oUserData);
            if (RoiMode == ROIMode.None)
            {
                point_decide = null;
                point_move = null;
        ...
        public bool StartRoi_Rectangle1(double row1, double col1, double row2, double col2, ICallbackRoiRectangle1 rc, object user)
        {
            if (RoiMode != ROIMode.None)
                return false;
